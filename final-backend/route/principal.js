...
app.post(
  ...
  // admin,
  async (req, res) => {
    let { error } = principalSchema.validate(req.body);
    console.log(req);
...
app.post("/principal-sign-in", async (req, res) => {
...
app.post("/find-principal", async (req, res) => {
  ...
  if (!user) {
    return res.status(400).send("No Principal Found");
  }
  res.status(200).send(user);
});

app.post("/get-principals-grade", async (req, res) => {
  const user = await Principal.find({
